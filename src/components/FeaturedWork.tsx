    ...
    <section className="py-32 relative" ref={ref}>
      <div className="container px-6">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-primary font-body text-sm tracking-[0.2em] uppercase mb-4">Highlights</p>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {featured.map((item, i) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
            >
              <div className="absolute inset-0 rounded-2xl p-px bg-gradient-to-br from-primary/50 to-accent/50 opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-0" />
              <div className="relative rounded-2xl overflow-hidden bg-card m-px">
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={item.img} alt={item.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                </div>
                <div className="p-6">
                  <motion.h3 className="text-xl font-heading font-bold text-foreground" initial={{ x: -10 }} whileInView={{ x: 0 }}>
                    {item.title}
                  </motion.h3>
