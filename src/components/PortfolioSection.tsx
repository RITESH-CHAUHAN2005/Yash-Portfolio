...
const PortfolioSection = () => {
  ...
  return (
    <section id="portfolio" className="py-32 relative" ref={ref}>
      ...
      <div className="absolute top-1/3 right-0 w-96 h-96 bg-[#C8A96A]/5 rounded-full blur-[120px]" />
      <div className="container px-6 relative z-10">
        <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-[#C8A96A] font-body text-sm tracking-[0.2em] uppercase mb-4">Works</p>
        ...
        <motion.div className="flex flex-wrap gap-3 justify-center mb-12" initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.3 }}>
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-body transition-all ${filter === cat ? "bg-[#C8A96A] text-white shadow-lg hover:shadow-xl hover:bg-[#B89655]" : "glass-card text-[#6B6B6B] hover:text-[#1C1C1C]"
                }`}
            >
              {cat}
            </button>
            ...
            {filtered.map((project, i) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                  <span className="text-xs text-[#C8A96A] font-body uppercase tracking-wider">{project.category}</span>
                  <h3 className="text-lg font-heading font-bold text-[#1C1C1C]">{project.title}</h3>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
      ...
      <AnimatePresence>
        {selected && (
          ...
          >
            ...
            >
              <button onClick={() => setSelected(null)} className="absolute top-4 right-4 z-10 w-10 h-10 glass-card rounded-full flex items-center justify-center text-[#1C1C1C] hover:text-[#C8A96A] transition-colors">
                <X className="w-5 h-5" />
              </button>
              <img src={selected.img} alt={selected.title} className="w-full max-h-[60vh] object-cover" />
              <div className="p-8">
                <span className="text-[#C8A96A] text-xs uppercase tracking-[0.2em] font-body">{selected.category}</span>
                <h3 className="text-3xl font-heading font-bold text-[#1C1C1C] mt-2 mb-3">{selected.title}</h3>
                <p className="text-[#6B6B6B] font-body text-lg">{selected.desc}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
